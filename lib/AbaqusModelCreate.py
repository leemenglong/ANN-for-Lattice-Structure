...
def output1(name, Location, element):
    with open("%s-Location.txt" % name, 'w', encoding='utf-8') as f:
        for line in Location:
            f.write(line + '\n')

    with open("%s-element.txt" % name, 'w', encoding='utf-8') as f:
        for line in element:
            f.write(line + '\n')
    f.close()
    return 0
...
path = 'F:\\MaterialsMachineLearning\\abaqus\\Job-BJX-3\\'  # 项目存在的绝对路径
os.chdir(path)
...
for index in range(num_index):
    for pop in range(1, num_group + 1):
        ...
        Location[8:, :] = np.random.random((pop, 3)) * [LengthX, LengthY, LengthZ]
        # Location 每个节点位置
        ...
        for i in range(np.size(Location, axis=0)):
            ...
            s2 = "%s,  " % s2
            s3 = "%s" % s3
            strLocation.append(s1 + s2 + s3)
        for i in range(np.size(element, axis=0)):
            s1 = "%d,  " % element[i, 0]
            s2 = "%d" % element[i, 1]
            s1 = "%s" % s1
            s2 = "%s" % s2
            strElement.append(s1 + s2)

        mdb.saveAs(pathName=pathName)

        output1(path, strLocation, strElement)
